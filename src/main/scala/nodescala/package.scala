...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    /** Returns a future that is always completed with `value`.
      */
    def always[T](value: T): Future[T] = f.successful(value)

    /** Returns a future that is never completed.
      *
      * This future may be useful when testing if timeout logic works correctly.
      */
    def never[T]: Future[T] = f {
      blocking {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] =
      fs match {
        case ft :: fts => ft.flatMap(x => all(fts).flatMap(xs => f(x :: xs)))
        case Nil => Future(Nil)
      }
    ...
    /** Given a list of futures `fs`, returns the future holding the value of the future from `fs` that completed first.
      ...
      */
    def any[T](fs: List[Future[T]]): Future[T] = f.firstCompletedOf(fs)

    /** Returns a future with a unit value that is completed after time `t`.
      */
    def delay(t: Duration): Future[Unit] = f {
      blocking {
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      s.p.tryCompleteWith(f(s.cancellationToken))
      s
    }
  }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    /** Returns the result of this future if it is completed now.
      ...
      */
    def now: T = {
      def throwError = throw new NoSuchElementException
      if (f.isCompleted) {
        f.value.get match {
          case Success(x) => x
          case _ => throwError
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete { x => p.tryComplete(Try(cont(x))) }
      p.future
    }
